...
- **ADR-017 — Auth client-first + conversion pro 2026-05-25** (`docs/adr/0017-auth-client-first-conversion-pro.md`, statut Proposed) : tranche la contradiction historique entre `tukio_information_architecture.md` §I.2/I.3 (client-first + conversion) et `tukio_ux_flow_auth_accounts.md` Doc 4 §C.1-C.2 (inscription pro directe `?role=pro`). **Décision** : (1) un seul flux d'inscription sur l'apex, tout compte créé en `role=client` — pas d'inscription pro directe ; (2) le rôle `pro` est accordé **uniquement** par le wizard de conversion (KYC + Stripe + validation admin → `assignRealmRole('pro')`, `tukio:status` `pending_admin_review` → `active`) ; `seller.tukio.one` n'a aucune page d'auth, c'est un workspace gardé pour pros validés ; (3) routeur post-login **server-side sur l'apex `/auth/callback`** lisant les rôles de l'ID token : `admin*` → `admin.tukio.one`, sinon `pro` → `seller.tukio.one/{locale}/seller/dashboard` (priorité dual-rôle, IA §I.3), sinon `client` → `/{locale}/account/dashboard`. **Supersedes** UX Flow Doc 4 §C.1-C.2 + décision « dual-portal » du 2026-05-17 (portail d'inscription seller Story 1.11 abandonné). **Étend** ADR-009 (realm/clients inchangés) et s'appuie sur ADR-016 (apex unifié, cookie `.tukio.one`). **Conséquences sur le code** : déprécier puis retirer `POST /v1/auth/pro/register` (Story 1.3c, `gateway-api auth-pro.controller.ts:59`) ; conserver le `ProConversionWizard` comme chemin canonique ; **construire** le routeur post-login (inexistant — d'où le bug « tout le monde atterrit pareil »). **Suite** : `/bmad-correct-course` sur Stories 1.3c (déprécation→retrait), 1.4/1.6 (redirect login + email-verify) + nouvelle story routeur post-login.
- **ADR-018 — Inscription sur page Keycloak themée + social IdP 2026-05-25** (`docs/adr/0018-registration-keycloak-hosted-social-idp.md`, statut Proposed) : aligne l'inscription sur le login. Aujourd'hui le login redirige vers Keycloak mais l'inscription est un **formulaire local Next.js** (Story 1.2 : `SignUpForm` + `POST /v1/auth/customer/register` + Admin API) — incohérent et divergent du doc UX §B.1. **Décision** : (1) l'inscription bascule sur la **page Keycloak themée** (`register.ftl`, thème `tukio`, `registrationAllowed`/`verifyEmail` déjà actifs) via redirect ; (2) déprécier puis retirer le form local + endpoint + contracts `register-customer` ; (3) rôle `client` auto-assigné en composant `client` dans `default-roles-tukio` (préserve client-first ADR-017) ; (4) **vérification email déléguée au flow natif Keycloak** → supersede l'essentiel de Story 1.6 ; (5) capture données : CGU via required-action native KC + consentement marketing en champ custom `register.ftl` ; **acquisition UTM/`tk_acq` abandonnée** (attribution via Plausible + waitlist) ; (6) **social IdP Google + Microsoft** provisionnés dans le realm (Apple différé). **Conséquence majeure** : inversion backend — `user_profile` créé **réactivement** sur événement d'inscription Keycloak (event-listener SPI → NATS → identity-svc) au lieu du dual-write synchrone Story 1.2b. **Supersede** l'approche form local de Story 1.2 + le point « endpoint register unique » d'ADR-017 (modèle client-first conservé). **Suite** : `/bmad-correct-course` re-scope Stories 1.2 + 1.6 + story provisioning realm (default role, IdP, champ register.ftl) + listener `user_profile`-on-registration.
