   ...
   - **Réponse success** :
     ```json
     {
       ...
       "data": {
         ...
         "emailVerified": true,
         "postVerifyRedirect": "https://tukio.one/fr/account/dashboard",
         "role": "client",
         "tukioStatus": "active"
       },
       "meta": { ... }
...
9. **AC9 — Tests Playwright e2e flow FR/EN + axe-core (NFR48 + UX-DR10)** : Given `apps/public/e2e/auth/email-verify.spec.ts` (NEW), When je lance `pnpm --filter=apps/public test:e2e --grep "email verify"`, Then 10 tests :
   ...
   - **Test 9 (FR17 customer redirect)** : login Customer non-vérifié → naviguer `tukio.one/fr/cart/checkout` → middleware redirect `/fr/auth/verify-email-required?next=...` → vérifier page render avec email du user + CTA fonctionnel
